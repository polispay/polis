...
bool ComputeNextStakeModifier(const CBlockIndex* pindexCurrent, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    LogPrint("%s: prev modifier=0x%016" PRI64x" time=%s epoch=%u\n",
             __func__, nStakeModifier, DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nModifierTime).c_str(), nModifierTime);

    if (nModifierTime / nModifierInterval >= pindexPrev->GetBlockTime() / nModifierInterval)
    {
        LogPrint("%s: no new interval keep current modifier: pindexPrev nHeight=%d nTime=%u\n",
    ...
    if (nModifierTime / nModifierInterval >= pindexCurrent->GetBlockTime() / nModifierInterval)
    {
        ...
        LogPrint("%s : (v0.4+) no new interval keep current modifier: pindexCurrent nHeight=%d nTime=%u\n",
    ...
    for (int nRound=0; nRound<min(64, (int)vSortedByTimestamp.size()); nRound++)
    {
        ...
        if (GetBoolArg("-printstakemodifier", false))
            LogPrint("%s : selected round %d stop=%s height=%d bit=%d\n",
    ...
    if (GetBoolArg("-printstakemodifier", false))
    {
        ...
        LogPrint("%s : selection height [%d, %d] map %s\n", __func__, nHeightFirstCandidate, pindexPrev->nHeight, strSelectionMap.c_str());
    }
    LogPrint("%s : new modifier=0x%016" PRI64x" time=%s\n", __func__,
...
static bool GetKernelStakeModifierV05(unsigned int nTimeTx, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    {
        LogPrint("%s : stake modifier time: %s interval: %d, time: %s\n",
                 __func__,
...
bool CheckStakeKernelHash(unsigned int nBits, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTransactionRef& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
    ...
    ss << nStakeModifier;
    ss << nTimeBlockFrom << nTxPrevOffset << txPrev->nLockTime << prevout.n << nTimeTx;
