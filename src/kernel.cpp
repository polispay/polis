...
bool CheckStakeKernelHash(const CBlockIndex* pindexPrev, unsigned int nBits, unsigned int nTxPrevOffset, uint256 hashBlockFrom, int64_t blockFromTime,
    ...
    if (nTimeBlockFrom > Params().GetConsensus().nPosMitigationSwitchTime && (nValueIn < nMinimumStakeValue))
        return error("%s - stakeinput value less than minimum required (%llu < %llu), blockhash %s\n", __func__, nValueIn, nMinimumStakeValue, hashBlockFrom.ToString().c_str());
