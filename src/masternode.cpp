...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_15_MASTERNODE_LOCK_NUMBER) && mnodeman.size() < SPORK_15_MASTERNODE_LOCK_NUMBER_DEFAULT)
	return Log(strprintf("Masternode limit reached, cannot enable more masternodes", strTxHash, strOutputIndex, strService));
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    // should be at least not earlier than block when 1000 polis tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 polis tx -> 1 confirmation
